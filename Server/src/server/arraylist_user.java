...
public class arraylist_user {
	...
	public void list(String receiveString) {
		...
		try {
			...
			// 클라이언트에서 수신한 String을 /로 구분하여 배열에 저장
				...
				try {
					...
					try {
						...
						if (s == 1) {
							...
							r2 = stmt2.executeUpdate("insert into humanities" + "(StudentNum, Status, Date) value ('"
									+ tokens[1] + "','" + "1" + "','" + now + "')");

							for (int i = 0; i < list.size(); i++) {
								if (tokens[1].equals(list.get(i).getStudentNum()) == true) {
									r3 = stmt1.executeUpdate("insert into " + list.get(i).getID()
											+ "(Building, Status, Date) value ('humanities" + "','" + "1" + "','" + now
											+ "')");
								}
							}
							s = 0;
			...
			// 로그인 시, 아이디와 비밀번호를 받은 경우
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (tokens[1].equals(list.get(i).getID()) == true
							&& tokens[2].equals(list.get(i).getPW()) == true) {
			...
			// 회원가입 시, 기존의 다른 사용자와 학번이나 아이디가 동일할 경우
				...
				for (int i = 0; i < list.size(); i++) {
					if (tokens[1].equals(list.get(i).getStudentNum()) == true
							|| tokens[3].equals(list.get(i).getID()) == true) {
						message = "0";
						send.set_message(message);
						this.state = 1;
						break;
